...
Three skills for software teams: two audit/profiling skills, and one multi-agent plan orchestrator for Plan Mode. The audit skills run in `ultrathink` mode and produce evidence-backed, confidence-scored reports. The orchestrator fans out specialist sub-agents in parallel and compiles their findings.

> [!NOTE]
> **`application-audit` was retired in v1.5.0** in favour of the cross-stack [`software-assurance-audit-program`](../software-assurance-audit-program/) plugin. SAAP covers what `application-audit` did and adds performance, reliability, privacy, compliance (SOC 2 / ISO 27001 / PCI readiness), accessibility, AI/ML assurance, vendor risk, and more — and adapts to any stack rather than assuming Next.js + Supabase + Postgres. Existing `.anthril/` workspaces are migrated by `/software-assurance-audit-program:migrate-anthril` (the original `.anthril/` data is left intact). See [`engineering-os/software-assurance-audit-program/.docs/00-deployment-decisions.md`](../software-assurance-audit-program/.docs/00-deployment-decisions.md).
...
| 4 | `codebase-profiler` | Stack-agnostic codebase reconnaissance — architecture topology, dependency graph, code-quality metrics, security surface, test posture, infrastructure snapshot. Used as a precursor to `dead-code-audit` and `plan-orchestrator`, and as a building block for `software-assurance-audit-program:profile-application`. |
...
/software-development:codebase-profiler     ./
...
For full assurance audits (security, performance, reliability, privacy, compliance, accessibility, AI/ML), use [`software-assurance-audit-program`](../software-assurance-audit-program/) instead:

```
/software-assurance-audit-program:profile
/software-assurance-audit-program:audit --mode standard
```
...
> The former `application-audit` requirements (Supabase / Vercel / Sentry / GitHub / PostHog / Figma MCPs, `.anthril/` workspace, `/audit-proceed` resume command) moved with the skill to [`software-assurance-audit-program`](../software-assurance-audit-program/). The SAAP equivalent workspace is `.saap/` and the resume verb is `/software-assurance-audit-program:audit --resume <audit-id>`.
...
> Application-audit outputs moved to `software-assurance-audit-program` — see [its README](../software-assurance-audit-program/README.md) for the `.saap/` layout and the eight artefacts a complete audit produces (profile, capability matrix, agent reports, findings, evidence ledger, validation report, risk register, remediation roadmap, executive + technical + final reports).
...
```
official-business-plugins/engineering/software-development/
